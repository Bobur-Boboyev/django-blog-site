...
def blog_list(request):
    ...
    return render(request, 'blogs/blog_list.html', {'blogs': page_obj, 'query': query, 'page_obj': page_obj})


def blog_detail(request, pk):
    blog = get_object_or_404(Post, pk=pk)
    return render(request, 'blogs/blog_detail.html', {'blog': blog, 'user': request.user, 'comments': blog.comments.all(), 'request': request})
...
def feed(request):
    ...
    return render(request, 'blogs/feed.html', {
        'blogs': blogs
    })
...
def liked_blogs(request):
    ...
    return render(request, 'blogs/liked_blogs.html', {
        'blogs': blogs
    })